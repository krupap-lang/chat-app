...
//     try {
//       const { data } = await axios.get(`http://localhost:3000/messages/${roomId}`);
//       setMessages(data);
//     } catch (err) {
//       setMessages([]);
//     }
...
//     const roomId = [myId, selectedChat._id].sort().join('_');
...
//           <button onClick={handleLogout} style={{ marginLeft: '10px', color: 'red', border: 'none', background: 'none', cursor: 'pointer' }}>Logout</button>
//         </div>
//         <div style={{ flex: 1, overflowY: 'auto' }}>
//           {users.filter(u => u._id !== myId).map(user => (
//             <div key={user._id} onClick={() => joinChat(user)} style={{ padding: '15px', cursor: 'pointer', borderBottom: '1px solid #f9f9f9', background: selectedChat?._id === user._id ? '#e7f3ff' : 'transparent' }}>
//               {user.name}
//             </div>
//           ))}
//         </div>
...
//         {selectedChat ? (
//           <>
//             <div style={{ padding: '15px', background: '#fff', borderBottom: '1px solid #ddd' }}>{selectedChat.name}</div>
//             <div style={{ flex: 1, padding: '20px', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
//               {messages.map((m, i) => {
//                 const isMe = m.senderId === myId;
//                 return (
//                   <div key={i} style={{ alignSelf: isMe ? 'flex-end' : 'flex-start', background: isMe ? '#007bff' : '#fff', color: isMe ? '#fff' : '#000', padding: '10px', borderRadius: '10px', marginBottom: '10px', maxWidth: '70%' }}>
//                     {m.content}
//                   </div>
//                 );
//               })}
...
//               <input value={message} onChange={e => setMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()} style={{ flex: 1, padding: '10px', borderRadius: '5px', border: '1px solid #ddd' }} />
//               <button onClick={handleSend} style={{ marginLeft: '10px', padding: '10px 20px', background: '#007bff', color: '#fff', border: 'none', borderRadius: '5px' }}>Send</button>
//             </div>
//           </>
//         ) : <div style={{ margin: 'auto' }}>Select a user</div>}
//       </div>
//     </div>
//   );
// };
        ...
        {selectedChat ? (
          <>
            ...
            <div style={{ padding: '20px', background: '#fff', borderTop: '1px solid #ddd', display: 'flex' }}>
              <input value={message} onChange={e => setMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()} placeholder="Type a message..." style={{ flex: 1, padding: '10px', borderRadius: '5px', border: '1px solid #ddd', outline: 'none' }} />
